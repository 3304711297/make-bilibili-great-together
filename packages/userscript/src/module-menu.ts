...
export function getModuleEnabledSync(name: string): boolean {
  const stored = GM_getValue(`${ENABLED_PREFIX}${name}`);
...
export async function isModuleEnabled(name: string): Promise<boolean> {
  const stored = await GM.getValue<boolean>(`${ENABLED_PREFIX}${name}`);
  return stored ?? true;
}
...
export function initModuleMenu(mod: ModuleMeta): void {
  GM.registerMenuCommand(
    ...
    () => {
      GM_setValue(`${ENABLED_PREFIX}${mod.name}`, !getModuleEnabledSync(mod.name));
      // 与上游语义一致：开关在下次加载生效
