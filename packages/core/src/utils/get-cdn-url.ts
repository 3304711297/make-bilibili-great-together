...
// 上游此处为模块级懒加载单例 getCDNUtil（依赖全局 logger 单例）；
// core 内 logger 需注入，故由调用方（模块移植）自行 createCDNUtil(logger)。
export type CDNUtilInstance = ReturnType<typeof createCDNUtil>;
